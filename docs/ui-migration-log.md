...
**i18n +9; no api, route, gate or loop change.** A published post opened from the calendar kept only Duplicate and Delete Post. Delete implied the post would come down, but no network lets PostQueen remove a published post, so it stayed live. For a published post the button now reads "Delete from PostQueen", and its confirmation says the post stays live on the network while PostQueen removes it from the calendar and stops any repeats. A new primary action, Post again, opens a small dialog: the first post stays live, the same text and media go out again at the time you pick. It sends the same `PUT /posts/:id/date` with `action: 'schedule'` and `republish: true` that dragging a published post and choosing Reschedule sends. Duplicate becomes the secondary button, and "Go to post" opens the live post when it has a link. Unpublished posts are unchanged. `scripts/ui-migration-check.sh --update` changed `i18n.txt`.
