...
export const FarcasterProvider = () => {
  ...
  // error body along as the state and the login would fail on a mismatch
  // instead of saying so. Same guard as google.provider.tsx.
  const gotoLogin = useCallback(
    async (code: string) => {
      const response = await fetch('/auth/oauth/FARCASTER');
      const state = response.ok ? (await response.text()).trim() : '';
      if (!state) {
        toaster.show(
