...
Typed Node client for the [PostQueen](https://postqueen.ai) public API. Schedule posts, list and
delete them, upload media and read your connected channels across X/Twitter, LinkedIn, Instagram,
Facebook, TikTok, YouTube, Reddit, Bluesky, Mastodon, Discord and 30+ more networks.
...
| Method | Returns |
| --- | --- |
| `post(posts: CreatePostDto)` | Schedule a post — parsed JSON response |
| `postList(filters: GetPostsDto)` | List posts in a date range — parsed JSON response |
| `upload(file: Buffer, extension: string)` | Upload an image or video — parsed JSON with the hosted `path` |
| `integrations()` | List connected channels — parsed JSON response |
| `deletePost(id: string)` | Delete a post — the raw `Response`, so call `.json()` yourself if you need the body |
