...
**i18n +5 −2 (in: `publish_credits_confirm_title`, `publish_credits_balance_after`, `publish_credits_short_body`, `publish_credits_why`, `publish_credits_why_link`; out: `publish_credits_hint`, `publish_credits_to_publish`); gates: `billingEnabled` 64 → 65 (the confirmation reads it three times where the composer's price read it twice); no api, route or loop change.** The price no longer sits in the composer: the coin and amount next to Save as draft, the row in the stacked footer and the one on the phone's Schedule pane are gone. Instead, Schedule, Post now and Update price the post as it is about to be saved (`POST /posts/credits`, as before) and, when the balance has to cover something, ask before the save in a compact dialog: the credits it takes, the balance after, and one line saying why, which names the network and, when the post has a link, both prices. Its button is the action that opened it. With too few credits it says so with the balance, and offers Get more credits (Billing in a new tab, for whoever may open Billing) and Back instead. Nothing is asked for a draft, for Sets, with billing off, when nothing more is set aside (an edit that keeps its price), or when the price cannot be read; the server still refuses a short balance with the Payment Required dialog. The quote gains per channel its network (`identifier`), `links` and `rates` (a plain post and one with a link), priced by the provider's `creditCost` as before. There is nothing for this in the prototype; it follows the compact modal, the Are you sure? dialog's buttons and the credits card's tokens. New keys are in `en` only; other locales fall back to them. `scripts/ui-migration-check.sh --update` changed `i18n.txt` and `gates.txt`.
