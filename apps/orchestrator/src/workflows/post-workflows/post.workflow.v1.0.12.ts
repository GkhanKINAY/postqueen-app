...
export async function postWorkflowV1012({
  taskQueue,
  postId,
  organizationId,
  postNow = false,
  ...
  const markUnconfirmed = async (err: any, index = 0) => {
    ...
    await publishingNotice(
      post.organizationId,
      postsList[index].id,
      what === 'post' ? 'unconfirmed' : 'unconfirmed_comment'
    );
  };
  ...
  if (post.integration?.refreshNeeded) {
    await publishingNotice(post.organizationId, post.id, 'reconnect');
  ...
  if (post.integration?.disabled) {
    await publishingNotice(post.organizationId, post.id, 'disabled');
  ...
  if (post.integration?.inBetweenSteps) {
    await publishingNotice(post.organizationId, post.id, 'unfinished');
  ...
  const resolvePending = async (
    ...
    if (index === 0) {
      await publishingNotice(post.organizationId, post.id, 'processing');
    }
      ...
      try {
        ...
        if (handle.type === 'bad-body') {
          ...
          await publishingNotice(
            post.organizationId,
            postsList[index].id,
            index === 0 ? 'bad_body' : 'bad_body_comment',
            handle.message || ''
          );
          return false;
        }
  ...
  while (list.length > 0) {
    ...
    if (todo.type === 'repeat-post') {
      await startChild(postWorkflowV1012, {
