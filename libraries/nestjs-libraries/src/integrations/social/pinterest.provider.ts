...
import { mapPinterestSummaryMetrics } from '@gitroom/nestjs-libraries/integrations/social/post-metrics.map';
  ...
  async postsAnalytics(
    ...
    for (const postId of platformPostIds) {
      try {
        ...
        const data = await response.json();
        // Period metrics come back in summary_metrics; lifetime_metrics only
        // ever carries TOTAL_COMMENTS / TOTAL_REACTIONS (see postAnalytics)
        if (!data?.all?.summary_metrics) {
          continue;
        }
        rows.push(mapPinterestSummaryMetrics(postId, data.all.summary_metrics));
      } catch (err) {
