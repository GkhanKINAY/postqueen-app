...
**i18n +1 (`cancel_upload`); no api, route, gate or loop change.** A file on its way into the media library could not be stopped: the progress strip hides Uppy's own cancel button, the uploading tile had no control, and nothing but the error path ever cancelled. The tile now carries a × (Cancel upload) while its bytes are going up, in the same place and style as the finished tile's ⋯ button, and it removes the file from Uppy, which aborts its request on both the XHR and the multipart path; the tile goes with it. Once the bytes are in, the × is gone, because the server already has the file and there is no request left to abort. When the last file of a batch is removed, the uploader releases the composer lock and ends the caller's uploading state right away, and the empty `complete` that Uppy sends afterwards is ignored, so it cannot clear an upload started in between. The key is in `en` only; other locales fall back to it. `scripts/ui-migration-check.sh --update` changed `i18n.txt` only.
