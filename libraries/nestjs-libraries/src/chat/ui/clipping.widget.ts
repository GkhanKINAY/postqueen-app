export const CLIPPING_WIDGET_URI = 'ui://postqueen/clipping';

// MCP Apps (SEP-1865) widget: a single self-contained HTML document in the
// host's sandboxed iframe. A chat model can't wait between two status calls, a
// page can: clippingTool result (clippingId) -> clippingWidgetTicketTool
// (ticket, through the host) -> GET /clipping-widget/status every few seconds
// -> report the clips back to the model once they are done. Only plain GETs,
// so the browser never sends a CORS preflight
...
<style>
  :root { color-scheme: light dark; --bg: #ffffff; --fg: #0e0e0e; --muted: #6b6b6b; --border: #d9d9d9; --accent: #7c3aed; --ok: #1a7f37; --bad: #cf222e; }
  ...
  * { box-sizing: border-box; }
  ...
  /* pq-loop marks every indefinite animation, as in the app (scripts/ui-migration-check.sh) */
  @media (prefers-reduced-motion: reduce) { .pq-loop { animation: none !important; } }
...
(function () {
  ...
  request('ui/initialize', {
    appInfo: { name: 'postqueen-clipping', version: '1.0.0' },
    appCapabilities: {},
