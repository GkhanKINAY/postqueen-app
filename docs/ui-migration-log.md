...
**Create Post chrome: three panes, first comment, settings defaults.** Empty footer puts Add New Tag left and Next available / Save as draft / Select channels (disabled until a channel) on the right. After a channel: Repeat Post Every in the footer (full label); Tags / Notify in the header. Desktop is write | Post Preview | AI chat (Typefully-height AI, not docked under the editor). Maximize keeps the `p-[32px]` inset. First Comment stays behind a pink Add comment; extra comments reuse the posts array. Settings use FormSection + FormIcon and paint the real send defaults on first paint. Header bell path was a broken cubic (clapper-only); it is a Lucide bell again. Posts list All is mixed (including Posted) and newest-first; only Scheduled stays upcoming-only. i18n +1 (`your_comment_should_have_at_least_one_character_or_one_image`). `scripts/ui-migration-check.sh --update` wrote `i18n.txt`.
