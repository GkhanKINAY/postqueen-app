...
// The brand token has no alpha channel, so the tints are mixed in CSS.
const MARK_CLASS = [
  'bg-[color-mix(in_srgb,var(--brand)_25%,transparent)]',
...
const markClassName = (count: number, pending: boolean) =>
  clsx(
    ...
    pending
      ? 'bg-[color-mix(in_srgb,var(--brand)_40%,transparent)] underline decoration-pqBrand decoration-2'
...
export const PostContentClient: FC<{ postId: string; html: string }> = ({
  ...
  // A phone selects with a long press and then drags handles, neither of
  // which ends in a mouseup on the text, so a touch selection is read once it
  // has stopped changing instead.
  ...
  useEffect(() => {
    ref.current
      ...
      .forEach((mark) => {
        ...
        mark.classList.toggle('ring-pqBrand', on);
      });
  ...
  useEffect(() => {
    ...
    mark.classList.add('pq-loop', 'animate-pulse');
    const timer = setTimeout(
      () => mark.classList.remove('pq-loop', 'animate-pulse'),
      1500
    );
  ...
  const onMouseMove = useCallback((e: MouseEvent<HTMLDivElement>) => {
    ...
    // and a tap on a phone is not a pointer resting on the text.
    if (!root || e.buttons || touchSelecting.current) {
      setHint(null);
      return;
    }
    const rootRect = root.getBoundingClientRect();
  ...
  return (
    <div className="relative">
      <div
        ref={ref}
        className="preview-comment-cursor whitespace-pre-wrap text-[14px] leading-[1.5] text-pqText"
        onTouchStart={() => (touchSelecting.current = true)}
        onMouseDown={() => (touchSelecting.current = false)}
      ...
      {!!selection && (
        <button
          type="button"
          // On pointer down, before a tap can collapse the selection and take
          // this button away with it; the click stays for the keyboard.
          onPointerDown={(e) => {
            e.preventDefault();
            startComment();
          }}
