 ...
 *   `postsList`. Nothing ever touched those rows again: they sat in QUEUE for
 *   good, with no error and no notice, so the user's extra content vanished
 *   silently. They are now marked, and one notice covers the whole set. The
 *   composer hides the extra parts for such a channel, but not in global mode,
 *   where it cannot know which channel is about to receive them - which is how
 *   they get created in the first place.
 *
 * Everything below is v1.0.10's behaviour, unchanged.
 *
 * v1.0.9 plus a notice while the platform is still processing an accepted post.
 *
 * - When `post` comes back `pending`, the platform has taken the media but has
 *   not published it yet (TikTok is the case that matters: it returns a
 *   publish id and is then polled by `checkPostStatus`). v1.0.9 said nothing
 *   for that whole window, so the only thing the user ever saw was a success
 *   or an error minutes later. The run now sends one `info` notice when it
 *   starts polling - once per post, no email, since a digest of "still
 *   working" is noise.
 *
 * Everything below is v1.0.9's behaviour, unchanged.
 *
 * v1.0.9 plus a claim on the post, so that only one run can ever publish it.
 *
 * - Before anything acts on the post, the run claims it (`claimPost`, one
 *   conditional write). A run that finds the claim held by a run that is still
 *   going stops without touching the post. One that finds it held by a run that
 *   is gone — terminated because the post was edited mid-publish — cannot know
 *   whether that run reached the platform, so it marks the post unconfirmed and
 *   tells the user to check the account instead of publishing it again. The
 *   claim is released when the publish settles (PUBLISHED or ERROR).
 * - A run that finds the post no longer in QUEUE stops silently. v1.0.8 wrote
 *   'Already posted' as an ERROR there, which turned a live post red whenever
 *   the hourly sweep started a run just after the first one finished.
 * - A failed comment marks that comment, not the root post, which is already
 *   live by then.
 * - A channel whose setup was never finished (`inBetweenSteps`) fails once with
 *   a reason, instead of through every retry.
 *
 * v1.0.10 and below stay registered for the executions that started on them.
 */
...
// Written when the claim is held by a run that no longer exists.
const INTERRUPTED = 'A previous publish attempt was interrupted';
const SETUP_NOT_FINISHED = 'Channel setup not finished';
// Written on every part a channel without a `comment` implementation cannot
// publish. `changeState` stays deliberately quiet for this exact string - the
// single notice below speaks for the whole set - so the two must agree. It
// names no provider: which channels can comment is `isCommentable`'s answer,
// not something spelled out here.
const NOT_COMMENTABLE = 'This channel cannot post comments';
...
export async function postWorkflowV1011({
  taskQueue,
  postId,
  organizationId,
  postNow = false,
  ...
  // Published, failed or drafted since this run was started: nothing to do,
  // and nothing to say either (see the note at the top).
  ...
  // The posts this run publishes: the whole thread, or only the first item
  // when the channel takes no comments (decided below).
  let postsList = postsListBefore;
  ...
  // account before reposting manually and duplicating it. `index` is the item
  // of the thread that could not be confirmed: past the first one, the root
  // post is already live and must stay PUBLISHED.
  const markUnconfirmed = async (err: any, index = 0) => {
    const what = index === 0 ? 'post' : 'comment';
    await changeState(postsList[index].id, 'ERROR', err, postsList);
    await inAppNotification(
      post.organizationId,
      `We couldn't confirm your ${what} on ${capitalize(
        post.integration?.providerIdentifier
      )}`,
      `Your ${what} was sent to ${capitalize(
        post.integration?.providerIdentifier
  ...
  // Only the run holding the claim may publish the post or change its state.
  ...
  if (claim === 'orphaned') {
    await markUnconfirmed(INTERRUPTED);
    return;
  }
  ...
  // connected, but a step of connecting it (picking a page or an account) was
  // never finished, so no publish to it can work
  if (post.integration?.inBetweenSteps) {
    await inAppNotification(
      post.organizationId,
      `We couldn't post to ${post.integration?.providerIdentifier} for ${post?.integration?.name}`,
      `We couldn't post to ${post.integration?.providerIdentifier} for ${post?.integration?.name} because connecting it was never finished. Open the channel to finish connecting it, then try again.`,
      true,
      false,
      'info'
    );
  ...
  postsList = toComment ? postsListBefore : [postsListBefore[0]];

  // Everything past the first part is unpublishable on this channel, and that
  // is true whether or not the root goes on to succeed, so it is settled here
  // rather than left to a later branch that an early return could skip.
  const droppedParts = toComment ? [] : postsListBefore.slice(1);
  if (droppedParts.length) {
    for (const part of droppedParts) {
      await changeState(part.id, 'ERROR', NOT_COMMENTABLE, postsList);
    }
    ...
    await inAppNotification(
      ...
      `${capitalize(post.integration.providerIdentifier)} has no way to add ${
        droppedParts.length === 1 ? 'a comment' : 'comments'
      } to a post, so ${
        droppedParts.length === 1
          ? 'the extra part of this post was'
          : `the ${droppedParts.length} extra parts of this post were`
  ...
  const resolvePending = async (
    ...
    // The platform has the media but has not published it: say so once, before
    // the first check, so the wait is not silent. Comments resolve the same way
    // and are not worth their own notice.
    if (index === 0) {
      await inAppNotification(
        post.integration.organizationId,
        `Publishing your post on ${capitalize(
          post.integration.providerIdentifier
        )}`,
        `${capitalize(
          post.integration.providerIdentifier
        )} accepted your post and is still processing it. It will be confirmed here once it is live.`,
        false,
        false,
        'info'
      );
  ...
  for (let i = 0; i < postsList.length; i++) {
    ...
    for (const _ of iterate) {
      ...
      try {
        ...
        // for other errors, change state and inform the user if needed. The
        // item that failed, not the root: past the first item the root post is
        // already live.
        await changeState(postsList[i].id, 'ERROR', err, postsList);
  ...
  while (list.length > 0) {
    ...
    if (todo.type === 'repeat-post') {
      await startChild(postWorkflowV1011, {
