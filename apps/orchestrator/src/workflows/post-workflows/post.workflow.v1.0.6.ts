  ...
  for (let i = 0; i < postsList.length; i++) {
    ...
    for (const _ of iterate) {
      try {
        ...
        // THE reason this file exists as a new version.
        //
        // In v1.0.5 control fell out of the catch here and the loop simply ran
        // `postSocial` again — five passes, each activity already retrying
        // three times, so up to fifteen real posts to the customer's timeline.
        // There is no idempotency anywhere on that path: `postSocial` never
        // reads `releaseId` before calling the provider and passes no
        // idempotency key. The errors that took this branch were provider 5xx,
        // connection resets and read timeouts — precisely the cases where the
        // post most likely *did* land.
        //
        // `updatePost` is inside the same try, so a database blip while marking
        // a successful post PUBLISHED took this path too: the row went to ERROR
        // and the post went out a second time.
        //
        // Only a token refresh is worth another pass, and that branch `continue`s
        // above. Everything else stops here.
        return false;
      }
    }
