...
**api +1 (`/billing/credits/checkout`); i18n +18 (`billing_yearly_credits_note`, `credits_buy`, `credits_buy_more`, `credits_card_sub`, `credits_card_sub_plain`, `credits_card_sub_yearly`, `credits_checkout_failed`, `credits_expiring`, `credits_left_short`, `credits_pack_validity`, `credits_per_credit`, `credits_purchased`, `credits_save_percent`, `plan_n_credits_year`, `withdrawal_waiver_credits`, `withdrawal_waiver_required`, `withdrawal_waiver_yearly`, `withdrawal_waiver_yearly_required`); no route, gate or loop change.** Billing gains a Credits card under the plan cards, for an organization with a plan: the balance, when the soonest of it runs out, and three packs (100, 500 and 1,000 credits for $10, $45 and $80, each with its price a credit and what the larger ones save). Buying one opens Stripe's hosted checkout and comes back to Billing, which reads the balance again as the payment lands. A paid founding member gets the same card inside the founding-member surface, above the payment row. Bought credits last twelve months; a full refund or a dispute takes back what is left of that pack and leaves the plan alone. The EU asks for the customer's consent when something paid for starts at once, so a pack is bought, and a yearly plan subscribed to, only with a box ticked that asks for the credits right away and acknowledges the 14-day right to withdraw ends once they are used. On Billing the box sits above the plan cards while Yearly is shown; in the first checkout it stands where the payment form goes, and the form is not created until it is ticked. The server refuses either without it. With Yearly shown, plan cards and the checkout's list say "N credits a year, up front", since a paid year's credits arrive at once. The card follows the plan cards' shape and tokens; there is no credits card in the prototype. New keys are in `en` only; other locales fall back to them. `scripts/ui-migration-check.sh --update` changed `api.txt` and `i18n.txt`.
