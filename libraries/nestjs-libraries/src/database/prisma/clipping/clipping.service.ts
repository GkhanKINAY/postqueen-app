...
// What a clip step answers its workflow. A failure the processor marked as
// retryable is thrown instead, so the activity runs the job again
export type ClippingJobState = 'done' | 'failed';

const CREDITS_TYPE = 'clipping_minutes';
// A fetcher may only cut on whole seconds (Oxylabs does), so the window is
// fetched a little wider and the clip job makes the exact cut inside it
...
export class ClippingService {
  ...
  private processor = UploadFactory.createClippingProcessor();
  ...
  private async getJson<T>(key: string): Promise<T> {
    return JSON.parse(await text(await this.storage.readFile(key)));
  }

  // A processor that cannot run anything here answers the same on every
  // attempt, so the clipping stops with that reason instead of retrying
  private async process<T>(job: () => Promise<T>): Promise<T> {
    try {
      return await job();
    } catch (err) {
      if (err instanceof ClippingNotConfiguredError) {
        throw new ClippingStop('Clipping is not configured on this server.');
      }
      throw err;
    }
  }

  // A failure the processor marked as retryable (a network blip, a busy
  // worker): thrown as a plain error, so the activity runs the job again and
  // the workflow closes the clipping once the attempts are used up
  private retry(failure?: ProcessorFailure | null): never {
    throw new Error(
      `Clipping job failed and will be retried (${failure?.code || 'FAILED'})`
    );
  }
  ...
  // The processor only fetches from YouTube; anything else would cost a job
  // to be told so
  ...
  // plans to meter against: checkCredits answers a large number there, the same
  // way image generation treats it
  ...
  // The stored flag says a trial started; whether it still runs is derived from
  // the registration date, the way the auth middleware does for the app. The
  // MCP organization carries the stored flag only, so it is derived here for both
  private async onTrial(organizationId: string) {
    const org = await this._organizationService.getOrgByIdWithSubscription(
      organizationId
    );
    return (
  ...
  async startClipping(org: Organization, body: ClippingDto) {
    if (
      ...
      throw new HttpException('Clipping is not configured', 503);
    }
  ...
  private async ingestVideo(
    clipping: { id: string; url: string },
    minutes: number,
    audioOnly: boolean,
    progress: (stage: string) => void
  ) {
    const keys = this.keys(clipping.id);
    const job: IngestJob = {
      version: 1,
      type: 'ingest',
      reference: clipping.id,
      source: { url: clipping.url },
      ...(audioOnly
        ? {}
        : {
            transcript: {
    ...
    return this.process(() => this.processor.ingest(job, progress));
  }

  // A video that can't be analysed stops the whole clipping, so this throws
  // where the clip steps answer "failed". The minutes are charged here, as
  // soon as the real duration is known and before any media is paid for;
  // "transcribe" says the video had no captions and only the audio was stored
  async analyse(
    clippingId: string,
    progress: (stage: string) => void
  ): Promise<{ transcribe: boolean }> {
    ...
    // an attempt that already found untrustworthy captions (below) asks for
    // the audio only
    let result = await this.ingestVideo(
      clipping,
      minutes,
      clipping.status === 'transcribing',
      progress
    );
    ...
    // and nothing in the answer tells the two apart, so the job runs again for
    // the audio and the transcriber finds the language itself
    if (
      result.status === 'completed' &&
      result.transcript &&
      result.transcript.origin !== 'auto_generated'
    ) {
      ...
      result = await this.ingestVideo(clipping, minutes, true, progress);
    }

    if (result.status === 'failed') {
      if (result.failure?.retryable) {
        this.retry(result.failure);
      }
    ...
    // so a retried attempt charges once and reads the same balance, and two
    // clippings racing each other are both counted before either is let through
    const charged = Math.ceil(duration / 60);
    const credits = (
    ...
    if ((await this.balance(clipping.organizationId)) < 0) {
      throw new ClippingStop(
        `This video is ${charged} minutes long, more than the clipping minutes left on this account for this month.`
      );
    }
    ...
    return { transcribe: !result.transcript };
  }
  ...
  // Only the window of the clip is downloaded, never the whole video. A clip
  // that can't be fetched is recorded as failed and never throws, the other
  // clips of the video go on
  async fetchClip(
    clipId: string,
    progress: (stage: string) => void
    ...
    const result = await this.process(() =>
      this.processor.ingest(job, progress)
    );
  ...
  async renderClip(
    ...
    // a retry after the media was saved must not render and save it twice
    ...
    const result = await this.process(() => this.processor.clip(job, progress));

    const rendered = result.clips?.[0];
    const failure = rendered?.failure || result.failure;
    ...
    if (rendered?.status !== 'completed') {
      if (!failure) {
        console.error('Unexpected render result:', JSON.stringify(result));
      }
      await this.failClip(
        clipId,
        failure
    ...
    // the media id is stored before anything else can fail, so a retry
  ...
  async failClip(clipId: string, error: string, customer = false) {
    const clip = await this._clippingRepository.getClipById(clipId);
    // the first reason is the specific one (what the processor answered)
  ...
  async failClipping(clippingId: string, error: string, customer = false) {
    ...
    // attempt that made it died before it could store creditsId
    if (!rendered) {
